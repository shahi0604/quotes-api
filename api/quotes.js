...
// Root route
app.get('/', (req, res) => res.send('Quotes API is running!'));
...
app.post('/quotes', (req, res) => {
  ...
  const newQuote = { id: quotes.length ? quotes[quotes.length - 1].id + 1 : 1, author, quote };
...
app.delete('/quotes/:id', (req, res) => {
  const id = parseInt(req.params.id);
  const index = quotes.findIndex(q => q.id === id);
  if (index !== -1) res.json(quotes.splice(index, 1)[0]);
  else res.status(404).json({ message: "Quote not found" });
});
